...
/// После того, как t1 дропнулось, t2.ptr становится висящей ссылкой, указывая на память, которая не кому не принадлежит.
...
/// 
/// Мьютекс из стандартной библиотеки имеет минимальный оверхэд так как он сразу сбрасывается.
/// В современных процессорах есть специальные инструкции, которые позволяют это оптимизировать.
/// 
...
/// 
/// Что можно сделать:
/// 1) Добавить конструктор из трех точек. 
/// Можно сделать derive copy например для 
/// new_triangle = {
///     (1, 1),
///     ..old_triangle,
/// }
